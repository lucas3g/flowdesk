...
void main() {
  ...
  test('salva, atualiza e exclui um layout do usuário', () async {
    ...
    final saved = (await repository.saveLayout(custom)).getOrElse(
      (f) => fail(f.message),
    );
    ...
    final updated = (await repository.saveLayout(
      saved.copyWith(
        name: 'Renomeado',
        regions: [
          const LayoutRegion(name: 'Única', x: 0, y: 0, width: 100, height: 100),
        ],
      ),
    ...
    expect(
      orphanRegions.any((r) => r.layoutId == updated.id),
      isFalse,
    );
  });
  ...
  test('persiste a associação de app da região', () async {
    ...
    final saved = (await repository.saveLayout(layout)).getOrElse(
      (f) => fail(f.message),
    );
    ...
    final region = reloaded
        .firstWhere((l) => l.id == saved.id)
        .regions
        .single;
  ...
  test('setFavorite alterna o favorito', () async {
    ...
    expect(
      reloaded.firstWhere((l) => l.id == first.id).isFavorite,
      isTrue,
    );
  });
}
